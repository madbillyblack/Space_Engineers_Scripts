...
public void DoorCheck(string secTag)
{
    ...
    {    
        IMyDoor door = doors[i] as IMyDoor;    
        string name = door.CustomName;
        if (name.Contains(secTag)&&name.Contains(SHIPTAG))    
        {
...
*/
public void EmergencyLights(string secTag)
{
    List<IMyTerminalBlock> whiteLights = new List<IMyTerminalBlock>();
    GridTerminalSystem.SearchBlocksOfName(secTag + " " + WHITE_TAG, whiteLights);
...
*/
public void OpenLock(string secTag)
{
    DoorClose(secTag);
    EmergencyLights(secTag);

    List<IMyTerminalBlock> alarms = new List<IMyTerminalBlock>(); 
    GridTerminalSystem.GetBlocksOfType<IMySoundBlock>(alarms); 
    for(int c = 0; c<alarms.Count; c++) 
    { 
        IMySoundBlock alarm = alarms[c] as IMySoundBlock; 
    ...
    { 
        IMyTimerBlock timer = timers[j] as IMyTimerBlock; 
...
*/
public void CloseLock(string secTag)
{
    List<IMyTerminalBlock> vents = new List<IMyTerminalBlock>(); 
    GridTerminalSystem.GetBlocksOfType<IMyAirVent>(vents); 
    for(int i = 0; i<vents.Count; i++) 
...
*/
public void CycleLock(string secTag)
{
    List<IMyTerminalBlock> vents = new List<IMyTerminalBlock>(); 
    GridTerminalSystem.GetBlocksOfType<IMyAirVent>(vents); 
